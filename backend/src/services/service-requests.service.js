...
const create = async ({ petId, serviceType, scheduledAt, meetingAddress }, user) => {
  ...
  if (scheduled < minScheduled) throw new AppError(400, 'O agendamento deve ser feito com pelo menos 2 horas de antecedência');
...
const start = async (id, user) => {
  ...
  if (request.caregiverId !== user.id) throw new AppError(403, 'Apenas o cuidador atribuído pode iniciar o serviço');
...
const complete = async (id, user) => {
  ...
  if (request.caregiverId !== user.id) throw new AppError(403, 'Apenas o cuidador atribuído pode concluir o serviço');
